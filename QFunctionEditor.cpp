...
#else
static char _QFunctionEditor_cpp[] = "MRC HGU $Id$";
#endif
#endif
/*!
...
//----------------------------------------
//
QFunctionEditor::QFunctionEditor(QWidget *parent)
  : QWidget(parent),
    m_penRed(Qt::red,1),
...
//----------------------------------------
//
QFunctionEditor::~QFunctionEditor() {
...
//----------------------------------------
//
void QFunctionEditor::resizeEvent( QResizeEvent *e ) {
  int nWidth  = width();
  int nHeight = height();
...
//----------------------------------------
//
void QFunctionEditor::paintEvent(QPaintEvent *e ) {
...
//----------------------------------------
//
void QFunctionEditor::drawTicks(QPainter *pPainter) {
  int nWidth  = width();
  int nHeight = height();
...
//----------------------------------------
//
void QFunctionEditor::drawLimits(QPainter *pPainter) {
  int nWidth  = width();
  int nHeight = height();
...
//------------------------------------------
//
void QFunctionEditor::drawHistogram(QPainter *pPainter,int start,int end) {
...
//----------------------------------------------
//
void QFunctionEditor::drawColorTable(QPainter *pPainter, int start, int end)  {
...
//----------------------------------------------
//
void QFunctionEditor::drawColorMap(QPainter *pPainter, int start, int end) {
   if (m_pColorMapBuffer != NULL) {
    ...
    start_y = nHeight - m_nBorder_y - 1 - (int) (255.0 * (*m_pColorMapBuffer)[start*4+3]);
    
    for(x = start+1; x < end; x++) {
      i++;
      end_x = m_nBorder_x + i;
      end_y = nHeight - m_nBorder_y - 1 - (int) (255.0 * (*m_pColorMapBuffer)[x*4+3]);
      pPainter->drawLine(start_x, start_y, end_x, end_y);
      start_x = end_x;
      start_y = end_y;
    }
    ...
    start_x = m_nBorder_x + start;
    start_y = nHeight - m_nBorder_y - 1 - (int) (255.0 * (*m_pColorMapBuffer)[start*4+2]);

    for(x = start+1; x < end; x++) {
      i++;
      end_x = m_nBorder_x + i;
      end_y = nHeight - m_nBorder_y - 1 - (int) (255.0 * (*m_pColorMapBuffer)[x*4+2]);
      pPainter->drawLine(start_x, start_y, end_x, end_y);
      start_x = end_x;
      start_y = end_y;
    }
    ...
    start_x = m_nBorder_x+start;
    start_y = nHeight - m_nBorder_y - 1 - (int) (255.0 * (*m_pColorMapBuffer)[start*4+1]);

    for(x = start+1; x < end; x++) {
      i++;
      end_x = m_nBorder_x + i;
      end_y = nHeight - m_nBorder_y - 1 - (int) (255.0 * (*m_pColorMapBuffer)[x*4+1]);
      pPainter->drawLine(start_x, start_y, end_x, end_y);
      start_x = end_x;
      start_y = end_y;
    }
    ...
    start_x = m_nBorder_x+start;
    start_y = nHeight - m_nBorder_y - 1 - (int) (255.0 * (*m_pColorMapBuffer)[start*4]);

    for(x = start+1; x < end; x++) {
      i++;
      end_x = m_nBorder_x + i;
      end_y = nHeight - m_nBorder_y - 1 - (int) (255.0 * (*m_pColorMapBuffer)[x*4]);
      pPainter->drawLine(start_x, start_y, end_x, end_y);
      start_x = end_x;
      start_y = end_y;
    }
...
//----------------------------------------------
//
void QFunctionEditor::drawFrame(QPainter *pPainter) {
...
//----------------------------------------------
//
void QFunctionEditor::mousePressEvent(QMouseEvent *m) {
  int point_x = m->x();
  int point_y = m->y();
  point_x -= m_nBorder_x;
  point_y -= m_nBorder_y;
  ...
  if (m->buttons()  & Qt::LeftButton > 0) {
    ...
    if ((point_x < 0) || (point_x > 255)) return; 
    if (point_y > 255) point_y = 255;
  ...
  } else if ((m->buttons() & Qt::MidButton > 0) || (m->buttons()& Qt::RightButton )) {
...
//----------------------------------------------
//
void QFunctionEditor::mouseReleaseEvent(QMouseEvent *m) {
  if (m->buttons() & Qt::LeftButton > 0 ) return;
  int point_x = m->x();
  int point_y = m->y();
  point_x -= m_nBorder_x;
  point_y -= m_nBorder_y;
...
//----------------------------------------------
//
void QFunctionEditor::resetTable(bool bSelected)
{
...
//----------------------------------------------------
//
void QFunctionEditor::DoLine(int &start_x, int &start_y, int &end_x, int &end_y) {
  bool bChanged = false;
  ...
  for(int x = x1; x != x2+xstep; x+= xstep) {
    ...
    i = 4*x;
	if (m_nInternalMode == INTERNAL_MODE_COLOR_TABLE) {
		if (m_nMode & 1) m_pColorTableBuffer[i  ] = (unsigned char)value;
		if (m_nMode & 2) m_pColorTableBuffer[i+1] = (unsigned char)value;
		if (m_nMode & 4) m_pColorTableBuffer[i+2] = (unsigned char)value;
		if (m_nMode & 8) m_pColorTableBuffer[i+3] = (unsigned char)value;
	} else {
                if (m_nMode & 1) m_pColorMapBuffer->set1Value(x*4, (float)value / 255.0F);
                if (m_nMode & 2) m_pColorMapBuffer->set1Value(x*4+1, (float)value / 255.0F);
                if (m_nMode & 4) m_pColorMapBuffer->set1Value(x*4+2, (float)value / 255.0F);
                if (m_nMode & 8) m_pColorMapBuffer->set1Value(x*4+3, (float)value / 255.0F);
        }
    y1 += ystep;
    bChanged = true;
  }
...
//----------------------------------------------------
//
void QFunctionEditor::mouseMoveEvent(QMouseEvent *m) {
  if ((m_nBorder_x < 0) || 
      (m_nBorder_y < 0)) return;
  int x,w;
...
//----------------------------------------------------
//
bool QFunctionEditor::ClipLine(int &p1_x, int &p1_y, int &p2_x, int &p2_y) {
  // check y values
  if (p1_y > 255) {
    p1_y = 255;
    if (p1_x > 255) {
...
//----------------------------------------------------
//
void QFunctionEditor::setHistogram(float *pHistogram, float fMaxVal) {
...
//
void 
QFunctionEditor::setTable(unsigned char *pTable) {
        m_pColorMapBuffer = NULL;
...
//----------------------------------------------------
//
void QFunctionEditor::setMap(SoMFFloat *pColorMap) {
	m_pColorTableBuffer = NULL;
