...
#else
static char _QFunctionEditor_h[] = "University of Edinburgh $Id$";
#endif
/*!
...
*
*		This file is based on OpenQVis QFunctionEditor.qt.H (see
*		notes bellow) and it was migrated from Qt3 to Qt4 by
*		Zsolt Husz together with extension to low/high cut off modes
* \ingroup	UI
*/
 ...
 *
 * 		The class QFunctionEditor provides a transfer function editor,
 * 		that supports both "color tables", that store RGBA values in
 * 		sequential order, and "component maps" that store the color
...
class QFunctionEditor: public QWidget
{
  ...
  public:
     ...
     *  				another widget, this widget becomes a
     *  				child window inside parent. The new
     *  				widget is deleted when parent is.
     * \param name 			Name of the widget. The name is sent
     ...
     *  				that contains the RGBA values.
     */
    void setTable(unsigned char *pTable);

    /*!
     * \brief 	slot function for setting the component maps to be edited
     * \param 	pR 			pointer an array of 256 float values,
     * 					that contains the red values.
     * \param 	pG 			pointer an array of 256 float values,
     * 					that contains the green values.
     * \param 	pB 			pointer an array of 256 float values,
     * 					that contains the blue values.
     * \param 	pA 			pointer an array of 256 float values,
     * 					that contains the alpha values.
     */
    void setMap(SoMFFloat *colorMap);

    /*! \brief	slot function for setting the histogram
     *  \param	pHistogram 		pointer an array of 256 float values,
     *  				that represents the histogram
     *  \param	fMaxVal 		the maximum float value contained in
     *  				the histogram.
     */
     ...
     */
    void setHistScale(float fScale)
    {
      m_fHistScale = fScale;
      /*repaint(FALSE);*/
      repaint();
     ...
     *  \param	bSelected 		If set to true, only the selected
     *  				color components are resetted,
...
signals:
  ...
  protected:
    ...
    /*! \brief	helper functions that clips a line against the valid area
     * 		in the editor
     *  \param	start_x 		x-coordinate of the starting point of
     *  				the line
     *  \param	start_y 		y-coordinate of the starting point of
     *  				the line
     *  \param	end_x 			x-coordinate of the end point of the
     *  				line
    ...
    enum InternalMode {
      ...
      INTERNAL_MODE_COLOR_TABLE,        /*!< The widget displays a RGBA color
                                             table */
      INTERNAL_MODE_COLOR_MAP	   	/*!< The widget displays separate
      					     component maps for R,G,B and A. */
    };

    /** @name Pens and Brushes
     * Predefined pens and color brushes for drawing the different parts
     * of the widget
     */
    //@{
    QPen	m_penRed;
    ...
    /*!
     * \brief	A pointer to the current color table (array of 256 unsigned
     * 		chars). This pointer is only valid, if m_nInternalMode is
     * 		equal to INTERNAL_MODE_COLOR_TABLE.
     */
    unsigned char *m_pColorTableBuffer;

    float      *m_pHistogram;	   	/*!< the current histogram as an
    					     array of 256 floats */
    float       m_fHistogramMax;   	/*!< maximum value in the histogram */
    float       m_fHistScale;	   	/*!< current scaling factor for the
    					     histogram */

    /*!
     * \brief 	pointer to the current colour map. These pointers are only
     * 		valid, if m_nInternalMode is equal to INTERNAL_MODE_COLOR_MAP.
     */
