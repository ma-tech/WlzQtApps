...
WoolzDynObject::WoolzDynObject(ObjectListModel * objectListModel) : WoolzObject(), m_objectListModel(objectListModel) {
  m_autoUpdate = false;
  m_srcObjID = -1;
  m_srcObjIDUsed = 0;
...
void WoolzDynObject::sourceChanged () {
  if ((m_autoUpdate && config.globalAutoUpdate())) {
     update(true);
  }
}

void WoolzDynObject::update ( bool force ) {
...
void WoolzDynObject::setAutoUpdate( bool newAutoupdate) {
  m_autoUpdate = newAutoupdate;
}

void WoolzDynObject::removedSourceObject() {
...
void WoolzDynObject::setSourceObj(WoolzObject *src) { 
...
bool WoolzDynObject::isAllowedSource(WoolzObject *src) {
...
void WoolzDynObject::setTypeFromSource () {
  WoolzObject *srcObj = sourceObj();
  if (srcObj)
     changeWoolzObjectType(srcObj->type());
}

WoolzObject* WoolzDynObject::sourceObj() {
  return m_objectListModel->getObject(m_srcObjID) ;
}

void WoolzDynObject::setupConnections(QObject *target) {
  WoolzObject::setupConnections(target);
}

bool WoolzDynObject::saveAsXmlProperties(QXmlStreamWriter *xmlWriter) {
...
bool WoolzDynObject::parseDOMLine(const QDomElement &element) {
...
bool  WoolzDynObject::needsUpdate() {
