...
#else
static char _SnapSurfaceDragger_cpp[] = "University of Edinburgh $Id$";
#endif
/*!
...
* 		chapter 8, see SnapSurfaceDragger.h.
* \ingroup      Controls
*/
...
startCB(
  ...
  SoDragger *dragger)
{
  SnapSurfaceDragger *myself = (SnapSurfaceDragger *) dragger;
  myself->dragStart();
}
...
drag()
{
  ...
  {
    // something went wrong this node should SoPickStyle added to the
    // scene graph by the constructor of WarperViewer
  ...
  SoPickStyle *psViews = (SoPickStyle *)root->getChild(0);
  if(!psViews->isOfType(SoPickStyle::getClassTypeId()))
  {
    // something went wrong this node should SoPickStyle added to the
    // scene graph by the constructor of WarperViewer
    fprintf(stderr,
	"SnapSurfaceDragger:: incorrect scene graph in drag(psViews)");
    return;
  }
  psViews->ref();
...
valueChangedCB(
  ...
  SoDragger *inDragger)
{
  SnapSurfaceDragger *myself = (SnapSurfaceDragger *) inDragger;
