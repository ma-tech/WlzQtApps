...
#else
static char _SegmentationWidget_cpp[] = "University of Edinburgh $Id$";
#endif
/*!
...
createNew()
{
  ...
  if(newTarget)
  {
	...
	new CreateWoolzObject(m_objectListModel, newTarget));
    m_object = newTarget;
  }
}
...
channelChanged()
{
  ...
  {
    channel = WoolzDynThresholdedObj::Red;
  }
  if(radioGreen->isChecked())
  {
    channel = WoolzDynThresholdedObj::Green;
  }
  if(radioBlue->isChecked())
  {
    channel = WoolzDynThresholdedObj::Blue;
  }
  if(m_object->channel() != channel)
  {
    m_objectListModel->addCommand(
	new WoolzDynThresholdedObjSetChannel(m_object, channel));
  }
}
...
{
  WoolzDynThresholdedObj* obj =
    qobject_cast<WoolzDynThresholdedObj*>(sender());
  if(!obj)
  {
    return;
  }
  ...
  WoolzObject* object)
{
  ...
  bool visible = object != NULL && !object->isWarpedPreRead() &&
                 object->isValuePreRead();
  ...
  WoolzObject* obj)
{
  connect(obj, SIGNAL(objectPropertyChanged()),
          this, SLOT(objectPropertyChanged()));
  connect(obj, SIGNAL(updated(bool)), this,
          SLOT(enableUpdate(bool)));
