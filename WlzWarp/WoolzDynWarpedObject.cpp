...
#else
static char _WoolzDynWarpedObject_cpp[] = "University of Edinburgh $Id$";
#endif
/*!
...
warp()
{
  ...
  if(errNum == WLZ_ERR_NONE)
  {
    if(!srcObj->isMesh())
    {
      m_obj = m_woolzTransform->Transform(srcObj,
...
setupConnections(QObject *target)
{
  ...
  connect(target, SIGNAL(updateAllWarpedSignal(bool)),
  ...
  bool force)
{
  if((m_autoUpdate && config.globalAutoUpdate() &&
      config.globalWarpUpdate() && needsUpdate()) || force)
  {
