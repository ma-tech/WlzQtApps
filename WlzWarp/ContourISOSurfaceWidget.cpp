...
#else
static char _ContourISOSurfaceWidget_cpp[] = "University of Edinburgh $Id$";
#endif
/*!
...
createNew()
{
  ...
  if(newTarget)
  {
    ...
    m_objectListModel->addCommand(new CreateWoolzObject(m_objectListModel,
        newTarget));
    m_object = newTarget;
  }
}
...
subSampleChanged()
{
  ...
  m_objectListModel->addCommand(new WoolzDynISOObjSetSubsample(m_object,
      subsample));
}

void ContourISOSurfaceWidget::
  loadProperties(WoolzDynContourISO *object)
{
...
void ContourISOSurfaceWidget::
objectSelected(WoolzObject* object)
{
  if(!object)
  {
    return;
  ...
  pushButtonNew->setEnabled((object != NULL) && !object->isWarpedPreRead() &&
                            object->isValuePreRead());
  if(m_object)
  {
  ...
  bool visible = (object != NULL) &&
                 (object->isValue() || object->isContourPreRead());
  ...
  if(obj!=NULL)
  {
    ...
    connect(obj, SIGNAL(updated(bool)),
            this, SLOT(enableUpdate(bool)));
...
void ContourISOSurfaceWidget::
removedObjectSignal(WoolzObject* obj)
{
...
void ContourISOSurfaceWidget::
sourceObjectChanged(int index)
{
  WoolzObject*  sourceObject =
      qobject_cast<WoolzObject*>(qVariantValue<QObject*>(
      comboSourceObject->itemData(index)));
  if(m_object)
  {
    ...
    m_objectListModel->addCommand(
        new WoolzDynObjectSetSourceObj(m_object, sourceObject));
  }
  checkBoxAutoUpdate->setEnabled((sourceObject) ||
                                 (m_object != NULL && m_object->sourceObj()));
}
  ...
  bool enabled)
{
 ...
 {
   m_objectListModel->addCommand(
       new WoolzDynObjectSetAutoUpdate(m_object, enabled));
 }
}
  ...
  bool enabled)
{
  ...
  {
    m_objectListModel->addCommand(
        new WoolzDynISOObjSetBoundary(m_object, enabled));
  }
}
  ...
  bool enabled)
{
  ...
  {
    m_objectListModel->addCommand(
        new WoolzDynISOObjSetFilter(m_object, enabled));
  }
}
...
setShellSelection()
{
  if(m_object)
  {
    ...
    m_objectListModel->addCommand(
        new WoolzDynISOObjSetShellSelection(m_object, selection));
  }
}
