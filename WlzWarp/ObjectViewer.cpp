...
#else
static char _ObjectViewer_cpp[] = "University of Edinburgh $Id$";
#endif
/*!
...
void ObjectViewer::setLinkedPlaneTo(ObjectViewer * viewer)
{
  if(viewer != this)
  {
    ...
    if(m_clipPlaneManip)
    {
      ...
      {
        m_clipPlaneManip->plane.connectFrom(
	    &(m_linkedPlaneTo->m_clipPlaneManip->plane));
        m_clipPlaneManip->draggerPosition.connectFrom(
	    &(m_linkedPlaneTo->m_clipPlaneManip->draggerPosition));
      }
      else
      {
...
bool ObjectViewer::saveAsXml(QXmlStreamWriter *xmlWriter)
{
  ...
  xmlWriter->writeTextElement("LinkedViewerID",
      QString("%1").arg(m_linkedViewerID));
  xmlWriter->writeTextElement("LinkedPlaneID",
      QString("%1").arg(m_linkedPlaneID));
