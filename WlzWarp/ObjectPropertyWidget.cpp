...
#else
static char _ObjectPropertyWidget_cpp[] = "University of Edinburgh $Id$";
#endif
/*!
...
* \brief        Widget for selecting and manipulating Woolz Object common
* 		properties
* \ingroup      UI
*/
...
void ObjectPropertyWidget::
colourChanged()
{
  if(m_object)
  {
    m_objectListModel->addCommand(
	new ColourObject(m_object, colorpickerObject->currentColor()));
  }
}

void ObjectPropertyWidget::
addObjectSignal(WoolzObject* obj)
{
  comboObjectName->addItem(obj->name(), qVariantFromValue<QObject*>(obj));

  int index = comboObjectName->findData(qVariantFromValue<QObject*>(obj),
  ...
  WoolzObject* obj)
{
  ...
  loadProperties(qobject_cast<WoolzObject*>(
	qVariantValue<QObject*>(comboObjectName->itemData(index))));
}
  ...
  int index)
{
  ...
  WoolzObject * obj = qobject_cast<WoolzObject*>(
      qVariantValue<QObject*>(comboObjectName->itemData(index)));
  if(!obj)
  {
    return;
...
void ObjectPropertyWidget::
loadProperties(WoolzObject *object)
{
...
void ObjectPropertyWidget::objectSelected(WoolzObject* object)
{
  if(m_object != object)
  {
    int index = comboObjectName->findData(
	qVariantFromValue<QObject*>(object), Qt::UserRole, Qt::MatchExactly);
    if(index >= 0)
    {
      m_object = NULL;
      comboObjectName->setCurrentIndex(index);
...
void ObjectPropertyWidget::removeObject()
{
  ...
  {
    m_objectListModel->addCommand(
	new DeleteWoolzObject(m_objectListModel, m_object));
  }
...
void ObjectPropertyWidget::visibleChanged(bool checked)
{
  if(m_object)
    m_objectListModel->addCommand(
	new WoolzObjectChangeVisibility(m_object, checked));
}
