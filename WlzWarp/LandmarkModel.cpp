...
#else
static char _LandmarkModel_cpp[] = "University of Edinburgh $Id$";
#endif
/*!
  ...
  LandmarkModel::IndexType indexType)
{
  ...
  changePersistentIndex(
      createIndex(index, (index == sourceV)? 1 : (1 + element), 0),
      createIndex(index, (index == sourceV)? element : (2 * element), 0));
  //force update
  emit layoutChanged();
  ...
  QXmlStreamWriter *xmlWriter)
{
 ...
 xmlWriter->writeTextElement("BasisFunctionType",
                             (m_basisFnType == basis_IMQ)? "IMQ":"MQ");
 xmlWriter->writeTextElement("Delta", QString("%1").arg(m_delta));
 xmlWriter->writeTextElement("SnapToFitDist",
                             QString("%1").arg(m_snapToFitDist));
 xmlWriter->writeEndElement();
 return(true);
}
  ...
  QFile *file)
{
  ...
  for(i = 0; i < listPointPair.size(); i++)
  {
	...
	stream << ' ' << (*pp)[sourceV].vtZ;
      ...
      stream << ' ' << (*pp)[targetV].vtX-(*pp)[sourceV].vtX <<
                ' ' << (*pp)[targetV].vtY-(*pp)[sourceV].vtY;
      if(is3D)
      {
...
writeLandmarkDebug(
  QString filename)
{
  filename += QDate::currentDate().toString(
      "dd.MM_")+QTime::currentTime().toString("HH:mm:ss") +".wrp";
  QFile file(filename);
...
WlzBasisFnTransform *LandmarkModel::
getBasisTransform(
  WoolzObject *cMesh,
  WlzErrorNum& errNum,
  bool isSource)
{
  ...
  if(errNum == WLZ_ERR_NONE)
  {
    ...
    {
      basisFn = (useIMQ)?
	WlzBasisFnIMQ3DFromCPts(ns, source.d3, target.d3, m_delta,
	    basisTr->basisFn, cMesh->getObj()->domain.cm3, &errNum):
	WlzBasisFnMQ3DFromCPts(ns, source.d3, target.d3, m_delta,
	    basisTr->basisFn, cMesh->getObj()->domain.cm3, &errNum);
    }
    else
    {
      basisFn = (useIMQ)?
	WlzBasisFnIMQ2DFromCPts(ns, source.d2, target.d2, m_delta,
	    basisTr->basisFn, cMesh->getObj()->domain.cm2, &errNum) :
	WlzBasisFnMQ2DFromCPts(ns, source.d2, target.d2, m_delta,
	    basisTr->basisFn, cMesh->getObj()->domain.cm2, &errNum);
    }
    if(basisTr->basisFn)
    {
  ...
  const PointPair *pp) const
{
  ...
  if(!m_mesh->getObj())
  {
    //if no mesh loaded/computed yet
    return(true);
  }
