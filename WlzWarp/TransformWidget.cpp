...
#else
static char _TransformWidget_cpp[] = "University of Edinburgh $Id$";
#endif
/*!
...
createNew()
{
  ...
  if(newTarget)
  {
    ...
    m_objectListModel->addCommand(new CreateWoolzObject(
	  m_objectListModel, newTarget));
    m_object = newTarget;
  }
}
  ...
  WoolzObject* object)
{
  ...
  if(obj != NULL)
  {
    ...
    QList <WoolzObject*>  list = m_objectListModel->getObjects(
	true, true, false);
    for(int i = 0; i < list.size(); i++)
    {
      if(obj->isAllowedSource(list.at(i)))
      {
        comboSourceObject->addItem(
	    list.at(i)->name(), qVariantFromValue<QObject*>(list.at(i)));
      }
    }
    loadProperties(obj);
  }
  ...
  bool visible = object != NULL &&
                 (object->isValuePreRead() || object->isMeshPreRead());
...
void TransformWidget::sourceObjectChanged(int index)
{
  ...
  checkBoxAutoUpdate->setEnabled(
      (sourceObject) || (m_object != NULL && m_object->sourceObj()));
  ...
  WoolzObject * source)
{
  ...
  {
    int index = comboSourceObject->findData(
	qVariantFromValue<QObject*>(source), Qt::UserRole, Qt::MatchExactly);
  ...
  {
    comboSourceObject->insertItem(0,
	tr("*removed*"), qVariantFromValue<QObject*>(NULL));
