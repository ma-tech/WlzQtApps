...
#else
static char _WoolzTransform_cpp[] = "University of Edinburgh $Id$";
#endif
/*!
...
m_meshObject(NULL)
{
  ...
  connect(m_landmarkModel, SIGNAL(removedLandmark(int)),
          this, SLOT(update()));
  connect(m_landmarkModel, SIGNAL(addedLandmarkPair(int)),
          this, SLOT(update()));
  connect(m_landmarkModel,
  ...
  connect(this, SIGNAL(statusChange(QString, int)),
          parent, SLOT(statusChanged(QString, int)));
}
...
void WoolzTransform::
setMeshObject(WoolzObject *meshObject)
{
...
updateBasisTr()
{
  ...
  if(!basisTr)
  {
    ...
    basisTr = m_landmarkModel->getBasisTransform(
	m_meshObject, errNum, m_meshObject->type() == WoolzObject::source);
...
WlzObject* WoolzTransform::
Transform(
  WoolzObject *srcObj,
  const WlzInterpolationType interp,
  WlzErrorNum *perrNum)
{
  ...
  bool direct  = (m_meshObject->type() == WoolzObject::source) ==
                 (srcObj->type() == WoolzObject::source);
  if(errNum == WLZ_ERR_NONE)
  {
    if(direct)
    {
  ...
  if(errNum == WLZ_ERR_NONE)
  {
    ...
    result = WlzCMeshTransformObj(srcObj->getObj(),
	                          (direct)? transformObj: invertedtransformObj,
				  interp, &errNum); //volume transform
  ...
  WlzErrorNum *perrNum)
{
  ...
  if(errNum == WLZ_ERR_NONE && invertedtransformObj)        // mesh transform
  {
    ...
    result = WlzMakeMain((m_meshObject->is3D())? WLZ_CMESH_3D: WLZ_CMESH_2D,
	                 invertedtransformObj->domain, val, NULL, NULL,
			 &errNum);
