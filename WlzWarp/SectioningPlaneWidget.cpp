...
#else
static char _SectioningPlaneWidget_cpp[] = "University of Edinburgh $Id$";
#endif
/*!
...
QDockWidget(parent),
m_objectViewerController(objectViewerController),
m_viewer(NULL)
{
  ...
  m_fieldSensor = new SoFieldSensor(&SectioningPlaneWidget::fieldSensorCB,
      this);
}
...
void SectioningPlaneWidget::
changedLink()
{
  bool notLinked = !(qVariantValue<QObject*>(
	comboBoxLinked->itemData(comboBoxLinked->currentIndex())));
...
apply()
{
  ...
  ObjectViewer *link = qobject_cast<ObjectViewer*>(
      qVariantValue<QObject*>(comboBoxLinked->itemData(index)));
  if(link)
  {
  ...
  {
    cl->draggerPosition.setValue(
	doubleSpinBoxPositionX->value(),
	doubleSpinBoxPositionY->value(),
	doubleSpinBoxPositionZ->value());
    cl->plane.setValue(SbPlane(SbVec3f(
	    doubleSpinBoxNX->value(),
	    doubleSpinBoxNY->value(),
	    doubleSpinBoxNZ->value()),
	  doubleSpinBoxDist->value()));
  }
  m_viewer = temp;
}
  ...
  ObjectViewer* viewer)
{
  ...
  if(cl)
  {
    ...
    QList<ObjectViewer*> list = 
        m_objectViewerController->model()->getViewers();
    ...
    while(i.hasNext())
    {
      ...
      {
        comboBoxLinked->addItem(
	    cur_viewer->windowTitle(),
	    qVariantFromValue<QObject *>(cur_viewer));
      }
    }
    int index = comboBoxLinked->findData(
	qVariantFromValue<QObject *>(viewer->getLinkedPlaneTo()));
    if(index < 0)
    {
      index = 0;
    }
  ...
  connect(viewer , SIGNAL(addedClipPlane(SoClipPlane *)),
          this, SLOT(addedClipPlane()));
...
fieldSensorCB(
  ...
  SoSensor *)
{
  SectioningPlaneWidget *myself = (SectioningPlaneWidget *) inWidget;
