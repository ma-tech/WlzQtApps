...
#else
static char _ViewerListEditor_cpp[] = "MRC HGU $Id$";
#endif
#endif
/*!
...
ViewerListEditor::ViewerListEditor(QMdiArea * mdiArea, QWidget *widget ) :
                     QComboBox(widget), m_mdiArea(mdiArea) {
    populateList();
}

QVariant ViewerListEditor::viewer() const {
  return itemData(currentIndex());
}

void ViewerListEditor::setViewer(QVariant viewer) {
    ObjectViewer* viewerobj = qobject_cast <ObjectViewer*>(viewer.value<QObject *>());
    if (viewerobj) {
...
void ViewerListEditor::populateList() {
