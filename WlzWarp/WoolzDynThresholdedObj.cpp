...
#else
static char _WoolzDynThresholdedObj_cpp[] = "University of Edinburgh $Id$";
#endif
/*!
...
doUpdate()
{
  ...
  else
  {
    ...
    if(m_lowTh > 0)
    {
      ...
      greyV.v.ubv = m_lowTh;
      // NOTE: Temporary object must be keept alive, since is used as
      // thresholeded object
      tempObj = WlzAssignObject(
	        WlzThreshold(srcObj->getObj(), greyV, WLZ_THRESH_HIGH,
		             &errNum), NULL);
    }
    else
    {
    ...
    if(m_highTh < 255)
    {
      ...
      m_obj = WlzAssignObject(
	      WlzThreshold(tempObj, greyV, WLZ_THRESH_LOW, &errNum), NULL);
    }
    else
    {
...
bool WoolzDynThresholdedObj::
saveAsXml(QXmlStreamWriter *xmlWriter)
{
...
isValue()
{
  ...
  // is it really a value object, or the source is value and this object
  // is read from while but not computed yet.
