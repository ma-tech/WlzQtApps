...
#else
static char _PreferencesDialog_cpp[] = "University of Edinburgh $Id$";
#endif
/*!
  ...
  parent)
{
  ...
  m_origSourceViewerColour = settings.value(
      "viewer/source/color", QColor(0, 77, 51)).value<QColor>();
  m_origTargetViewerColour = settings.value(
      "viewer/target/color", QColor(0, 51, 77)).value<QColor>();
  m_origResultViewerColour = settings.value(
      "viewer/result/color", QColor(77, 0, 0)).value<QColor>();
  m_orig2DMarkerSize       = settings.value(
      "markers/2D/size", 1.00f).value<float>();
  m_orig3DMarkerSize       = settings.value(
      "markers/3D/size", 1.00f).value<float>();
  m_origSnapToFitDist      = settings.value(
      "markers/snaptofitdist", 16.00f).value<float>();

  m_origIsIMQ 		   = settings.value(
      "warping/type", QString("IMQ")).value<QString>() == "IMQ";
  m_origDeltaIMQ	   = settings.value(
      "warping/IMQ/delta", 0.1f).value<float>();
  m_origDeltaMQ  	   = settings.value(
      "warping/MQ/delta", 0.1f).value<float>();
  ...
  // tabWarping->setVisible(false); 	// do not use this tab
  					// could be used again to provide
...
void PreferencesDialog::apply()
{
  ...
  settings.setValue("markers/snaptofitdist",
      doubleSpinBoxSnapToFitDist->value());
...
void PreferencesDialog::reject()
{
  ...
  settings.setValue("warping/type",
      m_origIsIMQ ? QString("IMQ") : QString("MQ"));
