...
#else
static char _ObjectViewerModel_cpp[] = "University of Edinburgh $Id$";
#endif
/*!
...
ObjectViewer *ObjectViewerModel::
getViewer(
  const QModelIndex & index) const
{
  if(index.isValid())
  {
    return(qobject_cast<ObjectViewer*>(static_cast<QObject *>(
...
ObjectView *ObjectViewerModel::
getView(
  const QModelIndex & index) const
{
  if(index.isValid())
  {
    return(qobject_cast<ObjectView*>((QObject *)(index.internalPointer())));
  }
  return(NULL);
}
    ...
    else
    {
      switch(index.column())
      {
        case 0:
          return(viewer->windowTitle());
        case 6:
	  // transmits the view not the linked to field, let the
          // editor remove from the options the actual viewer
          return(qVariantFromValue<QObject*>(viewer));
      }
    }
  }
  return(QVariant());
...
Qt::ItemFlags ObjectViewerModel::
flags(
  const QModelIndex &index) const
{
  ...
  if(isView(index))
  {
    ...
    else
    {
	 ...
	 ((index.column()) != 3 || (view->getVisualisationTypesNo() > 1)))
      {
        flags |= Qt::ItemIsEditable;
      }
    }
  ...
  if(role == Qt::EditRole)
  {
    if(isViewer(index))
    {
      ...
      else if(index.column() == 6)
      {
        ...
        {
          linkedToViewer = qobject_cast <ObjectViewer*>(
	      value.value<QObject *>());
        }
...
int ObjectViewerModel::
getRowOfViewer(
  const ObjectViewer * viewer) const
{
  if(!viewer)
  {
    return(-1);
  ...
  QMdiSubWindow * window = qobject_cast<QMdiSubWindow *>(
      viewer->parentWidget());
  if(!window)
  {
    return(-1);
...
void ObjectViewerModel::
removedViewStart(ObjectView *view)
{
  ObjectViewer * viewer = qobject_cast<ObjectViewer *>(sender());
  if(viewer)
  {
  ...
  ObjectView *view)
{
  ...
  if(viewer)
  {
    ...
    if(row >= 0)
    {
	  ...
	  createIndex(row, 0, view),
	  createIndex(row, 2, view));  	//change name and colour
    }
    emit layoutChanged();
  }
}
...
void ObjectViewerModel::changedViewer()
{
  ...
  if(viewer)
  {
    ...
    {
      changePersistentIndex(
	  createIndex(row, 0, viewer),
	  createIndex(row, 7, viewer));
    }
    emit layoutChanged();
  }
}
...
void ObjectViewerModel::addedView(
  ObjectView *view)
{
  ObjectViewer * viewer = qobject_cast<ObjectViewer *>(sender());
  if(viewer && view)
...
QList<ObjectViewer *>  ObjectViewerModel::
getViewers() const
{
