...
#else
static char _WarperViewer_cpp[] = "MRC HGU $Id$";
#endif
#endif
/*!
                      ...
                      m_pickStyle(NULL), m_clipPlane(NULL), m_clipOn(true)
{
  ...
  if (m_clipLandmarkButton) {
      ...
      connect(m_clipLandmarkButton, SIGNAL(clicked(bool)), this, SLOT(buttonClipLandmarkClicked(bool)));
  }
...
void WarperViewer::notifyMouseEvent(SoEventCallback * event) {
  ...
  SoRayPickAction rp(m_viewer->getViewportRegion());
     ...
     if (deleteAction->isChecked()) {
      ...
      m_landmarkController->removeLandmark( ((LandmarkManip *) path ->getTail())->pointPair);
      event->setHandled();
     }
  }
}

void WarperViewer::setupLandmarkView(LandmarkModel::IndexType indexType) {
...
void WarperViewer::addedClipPlaneSlot(SoClipPlane * plane){
   if (!plane) {
...
void WarperViewer::buttonClipLandmarkClicked(bool on) {
   m_clipPlane->on = on;
   m_clipOn = on;
}

void WarperViewer::transparencyChanged(int transparency) {
