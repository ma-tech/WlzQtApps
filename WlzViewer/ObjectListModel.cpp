...
ObjectListModel::ObjectListModel (QObject * parent):  ObjectListModelAbstract(parent) {
}
...
void ObjectListModel::addAllObjectsTo(ObjectSimpleViewer *viewer) {
  Q_ASSERT(viewer);
...
void ObjectListModel::removeObjectFromStore(WoolzObject * object) {
      int row ;
...
bool ObjectListModel::removable(const QModelIndex & index) const {
  WoolzObject *object = getObject(index);
...
void ObjectListModel::objectChanged() {
  ...
  if (obj) {
    int row=-1;
      ...
      if (row>=0)
        changePersistentIndex(QModelIndex(), createIndex(row, columnCount(QModelIndex())-1, 0)); //update changed columns
        emit layoutChanged();
  }
}

void ObjectListModel::setSelectObject(WoolzObject *object) {
   emit objectSelected(object);
}

void ObjectListModel::selectAnObject() {
