...
void TransferFunctionWidget::xmlSkipElement(QXmlStreamReader &xmlReader) {
  xmlReader.readNext();
...
void TransferFunctionWidget::loadFromXml(QXmlStreamReader &xmlReader) {
  SoMFFloat colorMap;
  ...
  while (!xmlReader.atEnd()) {
    ...
    if (xmlReader.isStartElement() && xmlReader.name() == "lowCutOff") {
        m_object->transferFunction()->setLowCutOff(xmlReader.readElementText().toDouble());
      if (xmlReader.isEndElement())
         xmlReader.readNext();
    } else if (xmlReader.isStartElement() && xmlReader.name() == "highCutOff") {
        m_object->transferFunction()->setHighCutOff(xmlReader.readElementText().toDouble());
      if (xmlReader.isEndElement())
         xmlReader.readNext();
...
void TransferFunctionWidget::load(bool) {
  ...
  if (ext.toUpper()=="ICOL") {
    ...
    int low, high;
    in >>low>>high;
    m_object->transferFunction()->setHighCutOff((double)high*255);
    m_object->transferFunction()->setLowCutOff((double)low*255);
    m_object->transferFunction()->setColorMap(colorMap);
...
void TransferFunctionWidget::save(bool) {
  ...
  xmlWriter.writeTextElement("lowCutOff", QString("%1").arg(spinBoxLowCutOff->value()));
  xmlWriter.writeTextElement("highCutOff", QString("%1").arg(spinBoxHighCutOff->value()));
  for (int i=0; i<256 ; i++) {
