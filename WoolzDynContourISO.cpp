...
#else
static char _WoolzDynContourISO_cpp[] = "MRC HGU $Id$";
#endif
#endif
/*!
...
class ShellElement {
 public:
      ...
      int cnt;
      WlzGMShell    *cS;
      bool operator< (const ShellElement & elem) const {return cnt>elem.cnt;}
      ShellElement(int i, int c, WlzGMShell  *s): indx(i), cnt(c), cS(s) {}
 } ;

WoolzDynContourISO::WoolzDynContourISO(ObjectListModel * objectListModel) :
         WoolzDynObject(objectListModel), m_value(255), m_filter(false), m_boundary(true), m_subsample(4), m_selection(0xFFFF), m_shellNo(0), m_fullContourObject(NULL),
         max_no_checkboxes(15), m_valueUsed(0), m_filterUsed(true), m_boundaryUsed(false), m_subsampleUsed(4), m_selectionUsed(0xFFFF)
{
}

WoolzDynContourISO::~WoolzDynContourISO() {
  if (m_fullContourObject)
      WlzFreeObj(m_fullContourObject);
}

void WoolzDynContourISO::doUpdate () {
...
void WoolzDynContourISO::recomputeShells_intern() {
  if (m_fullContourObject == NULL || m_fullContourObject->domain.ctr == NULL || m_fullContourObject->domain.ctr->model == NULL)
      return;
  ...
  // 1) if m_fullContourObject is empty, it can't be copied and there is need shells exists so non can be removed.
  // 2) if all shells are selected, then use a refference of the original object
  ...
  if((fS = model->child) == NULL) // no shells. this should not be the case, we already checked
      return ;
  nS = fS->next;
  do {
      cS = nS;
...
void WoolzDynContourISO::recomputeFullContour(){
    ...
    if (m_subsample!=1) {
        WlzAffineTransform *transform =
            WlzAffineTransformFromScale(srcObj->is3D() ? WLZ_TRANSFORM_3D_AFFINE : WLZ_TRANSFORM_2D_AFFINE , 1.0/m_subsample, 1.0/m_subsample, 1.0/m_subsample, &errNum);
        if (errNum == WLZ_ERR_NONE) {
            tempObj = WlzAssignObject(WlzAffineTransformObj(srcObj->getObj(), transform, WLZ_INTERPOLATION_NEAREST, &errNum), NULL);
    ...
    if (errNum == WLZ_ERR_NONE) {
        if (m_boundary)
            ...
            dom.ctr = WlzContourObj(tempObj, WLZ_CONTOUR_MTD_ISO, m_value, 1, 0, &errNum);
     }
    ...
    if (errNum == WLZ_ERR_NONE && m_subsample!=1 ) {
        WlzAffineTransform *transform =
                WlzAffineTransformFromScale(srcObj->is3D() ? WLZ_TRANSFORM_3D_AFFINE : WLZ_TRANSFORM_2D_AFFINE, m_subsample, m_subsample, m_subsample, &errNum);
        if (errNum == WLZ_ERR_NONE) {
    ...
    if (errNum == WLZ_ERR_NONE)
        m_fullContourObject = WlzMakeMain(WLZ_CONTOUR, dom, val, NULL, NULL, &errNum);
...
bool WoolzDynContourISO::saveAsXml(QXmlStreamWriter *xmlWriter) {
...
bool WoolzDynContourISO::saveAsXmlProperties(QXmlStreamWriter *xmlWriter) {
...
bool WoolzDynContourISO::parseDOMLine(const QDomElement &element) {
...
bool  WoolzDynContourISO::needsUpdate() {
    return (m_value != m_valueUsed) || (m_filter != m_filterUsed) || (m_boundary  != m_boundaryUsed) ||
            (m_subsample != m_subsampleUsed) || (m_selection != m_selectionUsed) || WoolzDynObject::needsUpdate();
}

void WoolzDynContourISO::saveUsedParameters() {
    m_valueUsed      = m_value;
...
void WoolzDynContourISO::setupConnections(QObject *target) {
