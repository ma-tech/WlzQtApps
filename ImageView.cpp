...
#else
static char _ImageView_cpp[] = "MRC HGU $Id$";
#endif
#endif
/*!
* \file         ImageView.cpp
...
ImageView::ImageView ( QObject * parent, WoolzObject * object ): ObjectView(parent, object) {
  Q_ASSERT(obj->isValue() && !obj->is3D());

  if (obj->getObj()->values.core) {           //if it has value table
  ...
  if((errNum = WlzInitGreyScan(obj, &iwsp, &gwsp)) == WLZ_ERR_NONE) {
      ...
      while((errNum = WlzNextGreyInterval(&iwsp)) == WLZ_ERR_NONE) {
        ...
        case WLZ_GREY_UBYTE:
          for(i=iwsp.lftpos; i <= iwsp.rgtpos; i++){
            bufi=i-firsti;
            cbuffer[(bufi+bufj*width)*2] = gwsp.u_grintptr.ubp[i-iwsp.lftpos];
            cbuffer[(bufi+bufj*width)*2+1] = 255;
          }
          break;
        case WLZ_GREY_SHORT:
          for(i=iwsp.lftpos; i <= iwsp.rgtpos; i++){
            bufi=i-firsti;
            cbuffer[(bufi+bufj*width)*2] = gwsp.u_grintptr.shp[i-iwsp.lftpos];
            cbuffer[(bufi+bufj*width)*2+1] = 255;
          }
          break;
        case WLZ_GREY_INT:
          for(i=iwsp.lftpos; i <= iwsp.rgtpos; i++){
            bufi=i-firsti;
            cbuffer[(bufi+bufj*width)*2] = gwsp.u_grintptr.inp[i-iwsp.lftpos];
            cbuffer[(bufi+bufj*width)*2+1] = 255;
          }
          break;
...
void ImageView::generateSceneGraph ( bool /*bForce*/ ) {
  ...
  hints->vertexOrdering = SoShapeHints::CLOCKWISE;//COUNTERCLOCKWISE;
  ...
  if (obj->isCachedVisualisation())
    ...
    imgDat = malloc ( sz.vtX * sz.vtY * m_chanels);   // number of chanels plus alpha
    ...
    if (imgDat && sz.vtX*sz.vtY<10000000)
      texture->image.setValue(SbVec2s(sz.vtX, sz.vtY), m_chanels, (const unsigned char*)imgDat);
    objectColourChanged();
    SoTextureScalePolicy * pcy = new SoTextureScalePolicy;
    pcy->policy = SoTextureScalePolicy::FRACTURE;  // disable subsampling and shown non 2^n textures
    cache->addChild(pcy);
    cache->addChild(texture);
    if (sz.vtY>=sz.vtX) {
      SoTexture2Transform * texturetrans = 
            new SoTexture2Transform; //different Woolz <> Coin3D texture representation
