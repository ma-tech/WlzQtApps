...
ObjectToolDialog::ObjectToolDialog(ObjectListModelAbstract * model, QWidget *parent)
 : QDialog(parent), Ui::ObjectToolDialog() {
  ...
  if (model) {
    ...
    //objectList->setUniformRowHeights(true);
  }
...
void ObjectToolDialog::removeObject() {
  ObjectListModelAbstract* model = qobject_cast<ObjectListModelAbstract*>(objectList->model());
  if (!model)
    return;
