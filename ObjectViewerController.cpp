...
#else
static char _ObjectViewerController_cpp[] = "MRC HGU $Id$";
#endif
#endif
/*!
...
void ObjectViewerController::addSourceViewer(bool force)
{
    ...
    if ( sourceViewerCounter > 1 )
      sourceViewer->setWindowTitle(QString("Source view - %1").arg(sourceViewerCounter));
    else
      sourceViewer->setWindowTitle(QString("Source view"));

    setupObjectViewer(sourceViewer); //set up viewer

    lastSourceViewer = sourceViewer; // from now on this is the last access source viewer
    m_objectViewerModel->addedViewer(sourceViewer); //remove to be signal
}


void ObjectViewerController::addTargetViewer(bool force)
{
    ...
    if ( targetViewerCounter > 1 )
      targetViewer->setWindowTitle(QString("Target view - %1").arg(targetViewerCounter));
    else
      targetViewer->setWindowTitle(QString("Target view"));

    setupObjectViewer(targetViewer); //set up viewer

    lastTargetViewer = targetViewer; // from now on this is the last access source viewer
    //emit addedViewer(targetViewer); //signal that new viewer was added
}

void ObjectViewerController::addResultViewer(bool force)
{
    if (!force && lastResultViewer)
        return;

    WarperResultViewer *resultViewer = new WarperResultViewer(m_objectViewerModel, m_is3D);
    ...
    if ( resultViewerCounter > 1 )
      resultViewer->setWindowTitle(QString("Result view - %1").arg(resultViewerCounter));
    else
      resultViewer->setWindowTitle(QString("Result view"));

    setupObjectViewer(resultViewer); //set up viewer

    lastResultViewer = resultViewer; // from now on this is the last access source viewer

    m_objectViewerModel->addedViewer(resultViewer); //remove to be signal
}

void ObjectViewerController::setupObjectViewer(ObjectViewer *viewer)
{
    ...
    connect(this, SIGNAL(addObjectSignal(WoolzObject*)), viewer, SLOT(addObject(WoolzObject*)));
    connect(this, SIGNAL(removedObjectSignal(WoolzObject*)), viewer, SLOT(removedObject(WoolzObject*)));

    //connect listening to object changes
    connect(viewer, SIGNAL(changedObjectView(ObjectView *)), m_objectViewerModel, SLOT(changedObjectView(ObjectView *)));
    connect(viewer, SIGNAL(removedViewStart(ObjectView *)),  m_objectViewerModel, SLOT(removedViewStart(ObjectView *)));
    connect(viewer, SIGNAL(removedViewFinish(ObjectView *)), m_objectViewerModel, SLOT(removedViewFinish(ObjectView *)));
...
ObjectViewer* ObjectViewerController::activeWindow() {
  QMdiSubWindow *subWindow =  m_mainWindow->getWorkspace()->activeSubWindow();
  if (subWindow)
    return qobject_cast <ObjectViewer*> (subWindow->widget());
  return NULL;
}

void ObjectViewerController::updateActions() {
...
void ObjectViewerController::defaultLayout() {
  QSize size = m_mainWindow->getWorkspace()->size();
  size.setWidth(size.width()/3);
  if (lastSourceViewer) {
    QWidget *window = (QWidget *)lastSourceViewer->parent(); // MDI window, not the viewer
    window->showNormal();
    window->move(0,0);
    window->resize(size);
  }
  if (lastTargetViewer) {
    QWidget *window = (QWidget *)lastTargetViewer->parent(); // MDI window, not the viewer
    window->showNormal();
    window->move(size.width(), 0);
    window->resize(size);
  }
  if (lastResultViewer) {
    QWidget *window = (QWidget *)lastResultViewer->parent(); // MDI window, not the viewer
    window->showNormal();
...
void ObjectViewerController::defaultViewLinks() {
...
void ObjectViewerController::checkToSetupDefault() {
  if (resultViewerCounter == 0 && sourceViewerCounter == 1 && targetViewerCounter == 1) {
    addResultViewer(true);
    defaultLayout();
  }
}
