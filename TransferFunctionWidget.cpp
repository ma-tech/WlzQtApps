...
#else
static char _TransferFunctionWidget_cpp[] = "MRC HGU $Id$";
#endif
#endif
/*!
...
* \brief        Widget for manipulating segmented Woolz Object properties (see WoolzDynCutOffedObj)
* \ingroup      UI
*
*/
...
void TransferFunctionWidget::recomputeHistogram() {
...
void TransferFunctionWidget::objectSelected(WoolzObject* object) {
  if (m_object)
...
void TransferFunctionWidget::removedObjectSignal(WoolzObject* obj) {
  if (obj == m_object)
    objectSelected(NULL);
}


void TransferFunctionWidget::addObjectSignal(WoolzObject* obj) {
...
void TransferFunctionWidget::apply() {
    ...
    new TransferFunctionMapUpdate(m_objectListModel, m_object->ID(), m_colorMap, command);
    new TransferFunctionSetLowCutOff(m_objectListModel, m_object->ID(), spinBoxLowCutOff->value(), command);
    new TransferFunctionSetHighCutOff(m_objectListModel, m_object->ID(), spinBoxHighCutOff->value(), command);
...
void TransferFunctionWidget::selectChannels(bool) {
  if (checkBoxRed->isChecked() && checkBoxGreen->isChecked() && checkBoxBlue->isChecked())
  {
    if (checkBoxAlpha->isChecked())
    ...
    if (!checkBoxRed->isChecked() && !checkBoxGreen->isChecked() && !checkBoxBlue->isChecked() && checkBoxAlpha->isChecked())
...
void TransferFunctionWidget::tableChanged() {
  if (m_object)
      if (checkBoxAutoUpdate->isChecked())
         m_objectListModel->addCommand(new TransferFunctionMapUpdate(m_objectListModel, m_object->ID(), m_colorMap));
}

void TransferFunctionWidget::selectLuminance(bool enabled) {
    if (enabled) {
...
void TransferFunctionWidget::selectIntensity(bool enabled) {
   if (enabled) {
        checkBoxRed->setChecked(true);
        checkBoxGreen->setChecked(true);
        checkBoxBlue->setChecked(true);
...
void TransferFunctionWidget::selectAlpha(bool enabled) {
   if (enabled) {
...
bool TransferFunctionWidget::loadFromDOM(const QDomElement &element, QUndoCommand *command) {
  SoMFFloat colorMap;
...
void TransferFunctionWidget::load(bool) {
  QString filename = QFileDialog::getOpenFileName(NULL, tr("Load transfer function"), ".",
  ...
  if (!file.open(QFile::ReadOnly | QFile::Text)) {
    QMessageBox::warning(NULL, "Load transfer function", "Cannot open transfer function file.");
    return;
  }
  QUndoCommand * command = new QUndoCommand;
  ...
  if (ext.toUpper()=="ICOL") {
    ...
    int low, high;
    in >>low>>high;
    new TransferFunctionMapUpdate(m_objectListModel, m_object->ID(), colorMap, command);
    new TransferFunctionSetLowCutOff(m_objectListModel, m_object->ID(), (double)low*255, command);
    new TransferFunctionSetHighCutOff(m_objectListModel, m_object->ID(), (double)high*255, command);
  } else {
    ...
    if (!doc.setContent(&file, false, &errorStr, &errorLine, &errorColumn)) {
       QMessageBox::warning(NULL, "Load transfer function", QString("Parse error at line %1 column %2 :").arg(errorLine).arg(errorColumn) + errorStr);
     ...
     if (root.tagName() == "TransferFunction") {
       if (!loadFromDOM(root, command))
          QMessageBox::warning(NULL, "Load transfer function", QString("Error reading transfer function."));
     } else
       QMessageBox::warning(NULL, "Load transfer function", QString("Not a landmark file."));

    QApplication::restoreOverrideCursor();

    if (file.error()) {
      QMessageBox::warning(NULL, "Load transfer function", "Cannot read file.");
      delete command;
      return;
    }
  }
